...
- **마일리지 가치 계산기**: 
  - 모든 품목을 캐시 100% 사용 기준으로 효율 계산
  - 가장 효율 좋은 품목을 기준으로 자동 선택 (마일리지 사용 가능 품목 포함)
  - 마일리지 사용 품목과 최고 효율 품목 비교로 마일리지 가치 계산
  - 모든 마일리지 품목의 비율 통계 제공 (100% 초과 방지)
...
### CSS 개선 필요사항
- **단위 사용 불일치**: px와 rem이 혼재되어 사용됨 (gap, margin, padding 등)
- **간격 값 파편화**: 3px, 4px, 6px, 8px, 0.5rem, 1rem 등 표준화 필요
- **width 값 임의성**: 50px~200px 범위의 다양한 임의 값들 사용
- **CSS 변수 미사용**: 반복되는 색상, 간격, 크기 값들을 변수화하여 일관성 향상 필요
...
- **마일리지 환산 비율 100% 초과 문제**: 가장 효율 좋은 품목을 기준으로 비교하는 방식으로 해결
